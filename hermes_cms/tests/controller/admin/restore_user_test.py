...
from hermes_cms.controller.admin.restore_user import RestoreUser


@patch('hermes_cms.core.auth.session')
@patch.object(Auth, 'has_permission')
@patch('hermes_cms.controller.admin.restore_user.UserDB')
...
def test_delete_valid_user(user_db, permission_mock, session_mock):
    ...
    user_ctrl = RestoreUser()
    response = user_ctrl.put(user_id=1)

    assert response.status_code == 200
    assert user.set.call_args_list == [call(archived=False)]
