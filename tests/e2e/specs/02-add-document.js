    ...
    describe('Has Permission', function () {
        ...
        it('Can create a new page type document', function () {
            element.all(by.css('button')).get(0).click().then(function () {
                ...
                var menutitle = element(by.model('record.document.menutitle'));
                var url = element(by.model('record.document.url'));
                var type = element(by.model('record.document.type'));
                var published = element(by.model('record.document.published'));
                var show_in_menu = element(by.model('record.document.show_in_menu'));
                ...
                content.sendKeys(browser.params.add_page.content);
